...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="Robot label continuation (frozen concept 300): training and validation",
        show_header=False,
    )
    ...
    if not alive and state not in {"complete", "paused", "failed"}:
        table.add_row(
            "Recovery",
            "Worker absent; inspect evaluation.log; "
                ...
                atomic_json(
                    path,
                    {
                        **previous,
                        ...
                        "error": previous.get(
                            "error",
                            f"Worker exited with status {code}; see evaluation.log",
                        ),
                    },
                )
            return code
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_robot_label_continuation.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(
            table,
            title="Started Robot label continuation (frozen concept 300)",
            border_style="green",
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_robot_label_continuation/robot_v5_l4_seed0",
    )
    parser.add_argument("--session", default="grouped_robot_label_continuation")
