...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="Sequential concept-record intervention: frozen models",
        show_header=False,
    )
    ...
    table.add_row("Validation rows", str(heartbeat.get("offset", 0)))
    table.add_row(
        "Validation samples", str(heartbeat.get("validation_samples", "pending"))
    )
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_sequential_intervention.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(
            table,
            title="Started Sequential concept-record evaluation",
            border_style="green",
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_sequential_intervention/dsprites_l4_five_seeds",
    )
    parser.add_argument("--session", default="grouped_sequential_intervention")
