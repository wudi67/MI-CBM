"""Run the isolated Robot A/B quantum label-depth experiment on CUDA."""
...
def main() -> None:
    ...
    if args.plan_only:
        ...
        reuse_a0 = config.head_epochs == source.head_epochs
        report(
            f"[cyan]Robot Independent A/B quantum label depth[/cyan]\n"
            f"Source: {root}/long_label\n"
            f"Frozen concept epochs: {previous.concept_epochs}. "
            "A: 1 layer / 24 parameters; B: 5 layers / 112 parameters.\n"
            "Three label initializations per depth; paired first layer/readout.\n"
            f"epochs={config.head_epochs}; batch={pilot.batch_size}; "
            f"lr={pilot.learning_rate}; fixed sample-order seed={pilot.seed}.\n"
            f"Reuse completed A/init_0: {reuse_a0}; "
            f"new training cells: {5 if reuse_a0 else 6}.\n"
            "Train with true concept controls and retained B quantum states.\n"
            "Evaluate train/validation: normal / correct foot / correct all.\n"
