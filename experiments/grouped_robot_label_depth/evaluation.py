...
def verify_condition(job, role: str, name: str, mask: int) -> dict | None:
    ...
    original = job.parent.reference.quantum_raw[role]["measured"][
        "concept_probabilities"
    ]
    torch.testing.assert_close(
        raw["concept_probabilities"], original, atol=2e-6, rtol=2e-6
    )
    if job.reused and name in job.parent.reference.quantum_raw[role]:
        gold = job.parent.reference.quantum_raw[role][name]
        torch.testing.assert_close(
