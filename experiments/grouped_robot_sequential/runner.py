"""Run only five new Sequential label trainings and reuse audited comparisons."""
...
class Experiment(IndependentExperiment):
    def __init__(
        ...
        if resume:
            ...
            self.manifest = {
                ...
                **identity,
                ...
                "training": (
                    "Freeze admitted concept frontend; fresh label Adam; "
                    "measured X controls and exact Born-weighted label BCE"
                ),
                "pairing": (
                    "Same data, frontend, initial label parameters, RNG, sample "
                    "order and label updates as Independent and no-feedback"
                ),
                "selection": (
        ...
        self.reference: Reference = Reference(root, self.runtime, self.tick)
        self.save_once(
            "reference_lock.json",
        ...
        if tree_hash(self.initial) != tree_hash(expected):
            raise ValueError(
                "Sequential initial frontend/head/RNG differs from reference"
            )
        self.save_once(
        ...
        report(
            ...
            f"{len(config.seed_values)} new Sequential label trainings; "
            "112 active parameters"
        )
        ...
        atomic_json(
            self.output / "heartbeat.json",
            {
                ...
                "completed_cells": len(
                    list(
                        self.output.glob(
                            "seed_*/training/sequential/training_reference.json"
                        )
                    )
                ),
                "total_cells": len(self.config.seed_values),
                "completed_conditions": sum(
                    len(list(self.output.glob(f"seed_*/*/*/*/{name}")))
                    for name in ("evaluation_lock.json", "evaluation_reference.json")
                ),
                "total_conditions": len(self.config.seed_values) * 10,
                "test_evaluated": False,
                **self.details,
            },
        )
...
def run_experiment(
    ...
    for seed in shared.config.seed_values:
        ...
        if not (job.checkpoint_path.parent / "result.json").exists():
            if existing:
                raise ValueError("Locked Sequential training is missing")
    ...
    report(
        f"[green]Sequential comparison complete:[/green] {shared.output / 'summary.md'}"
    )
