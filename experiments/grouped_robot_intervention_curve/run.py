"""Evaluate every correction subset with frozen completed Robot models."""
...
def main() -> None:
    ...
    if args.plan_only:
        ...
        report(
            "[cyan]Robot: correction curves for 0 through 5 concepts[/cyan]\n"
            f"Source: {config.reference}\nSeeds: {config.seed_list()}\n"
            f"Role: {config.role}; samples: {len(sources.data['labels'])}\n"
            f"CUDA: {sources.runtime['device']}; batch: {config.eval_batch_size}\n"
            f"Training: none; 3 modes x 32 subsets per seed.\n"
            f"Development proxy: {config.development}; output: {output}\n"
            "Plan only: verifies existing artifacts; no new predictions or outputs."
        )
        return
    ...
    with (output / ".worker.lock").open("a", encoding="utf-8") as lock:
        ...
        atomic_json(
            output / "heartbeat.json",
            {
                ...
                "updated_at": utc_now(),
                "pid": os.getpid(),
                "total_conditions": 96 * len(config.seed_list()),
            },
        )
        shared = None
        previous = {}
