...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="Robot concept initialization: fixed four-layer circuit",
        show_header=False,
    )
    ...
    if not alive and state not in {"complete", "paused", "failed"}:
        table.add_row(
            ...
            "resume restores concept-circuit Adam and sample position",
        )
        state = "absent"
    return table, state
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_robot_concept_init.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(
            table,
            title="Started Robot concept-initialization experiment",
            border_style="green",
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_robot_concept_init/robot_v5_l4",
    )
    parser.add_argument("--session", default="grouped_robot_concept_init")
