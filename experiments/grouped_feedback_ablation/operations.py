...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="Paired feedback ablation: Joint pilot / Sequential repeats",
        show_header=False,
    )
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_feedback_ablation.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(
            table,
            title="Started paired feedback ablation",
            border_style="green",
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_feedback_ablation/dsprites_l4_paired",
    )
    parser.add_argument("--session", default="grouped_feedback_ablation")
