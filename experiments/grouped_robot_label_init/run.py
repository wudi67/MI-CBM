"""Compare three initializations of the same five-layer quantum label circuit."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--max-steps",
        type=int,
        help="Pause after this many NEW label optimizer updates",
    )
...
def main() -> None:
    ...
    if args.plan_only:
        ...
        depth, _, previous, pilot = config.sources()
        runtime = cuda_runtime(pilot.seed)
        reuse_uniform = config.head_epochs == depth.head_epochs
        report(
            f"[cyan]Robot Independent label initialization[/cyan]\n"
            f"Source: {root}/L5\n"
            f"Frozen concept epochs: {previous.concept_epochs}. "
            "All methods: 5 layers / 112 parameters.\n"
            "Uniform / small Gaussian / Gaussian with shifted readout; "
            "three indices each.\n"
            f"Gaussian sigma={config.sigma:.7f}; shifted readout center=pi/2.\n"
            f"epochs={config.head_epochs}; batch={pilot.batch_size}; "
            f"lr={pilot.learning_rate}; fixed sample-order seed={pilot.seed}.\n"
            f"Reuse completed three uniform cells: {reuse_uniform}; "
            f"new training cells: {6 if reuse_uniform else 9}.\n"
            "Train with true concept controls and retained B quantum states.\n"
            "Evaluate train/validation: normal / correct all concepts.\n"
            "Keep all 32 physical measurement branches; no test read/evaluation.\n"
            f"CUDA: {runtime['device']}\nOutput: {output}\n"
            "Plan only: no training or output creation. "
            "Full upstream audit occurs at startup."
        )
        return
