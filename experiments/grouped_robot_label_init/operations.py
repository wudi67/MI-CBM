...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="Robot quantum label initialization: fixed circuit",
        show_header=False,
    )
    ...
    for key, title in (
        ("validation", "Val normal label"),
        ("validation_true", "Val true-control label"),
        ("train_true", "Train true-control label"),
    ):
        if key in heartbeat:
            metric = heartbeat[key]["label"]
            table.add_row(
                title, f"accuracy={metric['accuracy']:.2%}, BCE={metric['bce']:.5f}"
            )
    if not alive and state not in {"complete", "paused", "failed"}:
        table.add_row(
            "Recovery",
            "Worker absent; inspect train.log; "
            "resume restores label-circuit Adam and sample position",
        )
        state = "absent"
    return table, state
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_robot_label_init.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(
            table,
            title="Started Robot label-initialization experiment",
            border_style="green",
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_robot_label_init/robot_v5_l4_head5",
    )
    parser.add_argument("--session", default="grouped_robot_label_init")
