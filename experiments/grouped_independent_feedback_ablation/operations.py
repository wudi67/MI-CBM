...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="Independent feedback ablation: frozen paired models",
        show_header=False,
    )
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_independent_feedback_ablation.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(
            table,
            title="Started Independent feedback ablation",
            border_style="green",
        )
    )
...
def main() -> None:
    ...
    parser.add_argument("--session", default="grouped_independent_feedback_ablation")
