...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="Robot P0 pilot: training and validation",
        show_header=False,
    )
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_robot_pilot.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_robot_pilot/robot_v5_l4_seed0",
    )
    parser.add_argument("--session", default="grouped_robot_pilot")
