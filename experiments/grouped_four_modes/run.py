...
def main() -> None:
    ...
    if args.plan_only:
        ...
        report(
            f"[cyan]Standard/Joint -> validation -> protocol lock -> test[/cyan]\n"
            ...
            f"Training: Standard, Joint +/- feedback; {config.epochs} epochs each.\n"
            f"Reuse seed 0: {not config.development}; 15 conditions per seed/stage.\n"
    ...
    with (output / ".worker.lock").open("a", encoding="utf-8") as lock:
        ...
        atomic_json(
            output / "heartbeat.json",
            {
                ...
                "updated_at": utc_now(),
                "pid": os.getpid(),
                "total_conditions": 30 * len(config.seed_list()),
            },
        )
        shared = None
        previous = {}
        try:
            ...
            run_experiment(shared, args.max_conditions, args.max_steps)
