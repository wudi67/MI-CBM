...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="dSprites four modes: training and final evaluation",
        show_header=False,
    )
    ...
    table.add_row("Sampling repetition", str(heartbeat.get("repetition", "pending")))
    table.add_row(
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_four_modes.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(
            table,
            title="Started dSprites four modes",
            border_style="green",
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_four_modes/dsprites_l4_five_seeds",
    )
    parser.add_argument("--session", default="grouped_four_modes")
