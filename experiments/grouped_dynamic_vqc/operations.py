...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(title="Grouped Dynamic VQC", show_header=False)
    ...
    if not alive and state not in {"complete", "paused", "failed"}:
        table.add_row("Recovery", "Worker absent; inspect train.log and resume.pt")
        state = "absent"
    return table, state
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    if "--resume" in arguments and not (output / "resume.pt").exists():
        raise FileNotFoundError("No resume.pt in selected output directory")
    output.mkdir(parents=True, exist_ok=True)
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_dynamic_vqc.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    # Completion leaves the terminal visible, without keeping a training worker alive.
    subprocess.run(
        [str(tmux), "set-window-option", "-t", session, "remain-on-exit", "on"],
        check=True,
    )
    monitor = shlex.join(
        [
    ...
    console.print(
        Panel(table, title="Started grouped VQC worker", border_style="green")
    )
...
def main() -> None:
    ...
    parser.add_argument("--session", default="grouped_dsprites_l4")
