...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(title="Classical MLP controls", show_header=False)
    ...
    if "validation" in heartbeat:
        ...
        table.add_row("Validation loss", f"{validation['loss']:.5f}")
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_mlp_controls.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(table, title="Started MLP control experiment", border_style="green")
    )
...
def main() -> None:
    ...
    parser.add_argument("--session", default="grouped_mlp_seed0")
