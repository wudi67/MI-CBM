...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(title="L4 Standard and control diagnostics", show_header=False)
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_control_diagnostics.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(table, title="Started VQC control diagnostics", border_style="green")
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_control_diagnostics/dsprites_l4_seed0",
    )
    parser.add_argument("--session", default="grouped_control_diagnostics")
