"""Train five complete Robot seeds with paired Independent feedback ablation."""
...
def parse_config(args: argparse.Namespace) -> Config:
    ...
    for key in ("reference", "concept_reference", "label_reference"):
...
def main() -> None:
    ...
    if args.plan_only:
        ...
        cells = 3 * len(config.seed_values)
        reused = 2 if config.reuse_seed0 else 0
        report(
            "[cyan]Robot Independent five-seed P0[/cyan]\n"
            f"Seeds: {config.seed_values}; {cells} jobs, {reused} reused.\n"
            "Circuit: 4 frontend layers + 5 label layers; 352 parameters.\n"
            "Front U[0, pi]; label sigma=0.00333333; initial readout RY +pi/2.\n"
            f"Concept epochs={config.concept_epochs}; "
            f"label epochs={config.head_epochs}.\n"
            f"Batch={pilot.batch_size}; eval={pilot.eval_batch_size}; "
            f"lr={pilot.learning_rate}.\n"
            "Paired: same frontend, initial label weights, Adam and order.\n"
            "Train/validation: measured, all-five correction, zero feedback.\n"
            f"CUDA: {runtime['device']}\nOutput: {output}\n"
            "Plan only: no training/output creation or test access. "
            "Full historical reuse audit runs during preflight/startup."
        )
        return
