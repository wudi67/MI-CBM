...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="Robot Independent: 4+5 layers, paired five-seed experiment",
        show_header=False,
    )
    ...
    table.add_row("Stage", heartbeat.get("stage", "pending"))
    ...
    table.add_row(
        "Epoch",
        f"{heartbeat.get('epoch_completed', 0)} / {heartbeat.get('epochs_total', '?')}",
    )
    table.add_row("Adam step", str(heartbeat.get("global_step", 0)))
    table.add_row(
    ...
    table.add_row("Processed rows", str(heartbeat.get("offset", 0)))
    table.add_row("Stage samples", str(heartbeat.get("stage_samples", "pending")))
    table.add_row("Device", heartbeat.get("device", "initializing CUDA"))
    ...
    if "reason" in heartbeat:
        table.add_row("Reason", heartbeat["reason"])
    if "validation" in heartbeat:
        metric = heartbeat["validation"]["concept"]
        table.add_row(
            "Val five concepts correct", f"{metric['all_concepts_accuracy']:.2%}"
        )
        table.add_row("Val mean concept accuracy", f"{metric['mean_bit_accuracy']:.2%}")
        table.add_row("Val concept NLL", f"{metric['joint_nll']:.5f}")
        if "label" in heartbeat["validation"]:
            table.add_row(
                "Val label accuracy",
                f"{heartbeat['validation']['label']['accuracy']:.2%}",
            )
    if not alive and state not in {"complete", "paused", "failed"}:
        table.add_row(
            "Recovery",
            "Worker absent; inspect train.log; "
            "resume restores full Adam and sample position",
        )
        state = "absent"
    return table, state
                ...
                atomic_json(
                    path,
                    {
                        **previous,
                        ...
                        "error": previous.get(
                            "error",
                            f"Worker exited with status {code}; see train.log",
                        ),
                    },
                )
            return code
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_robot_independent.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(
            table,
            title="Started Robot Independent five-seed experiment",
            border_style="green",
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_robot_independent/robot_v5_l4_head5_five_seeds",
    )
    parser.add_argument("--session", default="grouped_robot_independent")
    parser.add_argument("--watch", action="store_true")
    parser.add_argument("--since", type=float, default=0.0)
    args, remainder = parser.parse_known_args()
    output = args.out.resolve()
    if args.action in {"launch", "worker"}:
        from .run import (  # pylint: disable=import-outside-toplevel
            build_parser,
            parse_config,
        )
