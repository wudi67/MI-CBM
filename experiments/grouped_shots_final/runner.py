...
from .data import data_hashes, read_test, subset
from .evaluation import exact_metrics, infer, sample_repeats, sampled_metrics
from .protocol import CONDITIONS, Config, Sources, check_output, source_hashes
...
class Experiment:
    def __init__(self, config: Config, output: Path, resume: bool = False) -> None:
        ...
        if resume:
            ...
            self.manifest = {
                ...
                "training": "none; frozen Fusion L4 plus B5/readout L1",
                "evaluation": (
                ...
                "concept_reporting": (
                    "pre-correction frontend; finite metrics reported once per seed "
                    "using Independent/measured samples"
                ),
                "replication_unit": (
    ...
    @torch.no_grad()
    def states(self, seed: int) -> torch.Tensor:
        weights = load_checkpoint(self.sources.model_path(seed, "independent"))["model"]
    ...
    def evaluate_condition(
        ...
        if (root / "joint_lock.json").exists():
            ...
            if (
                ...
                != self.sources.pairing[str(seed)]["frontend_sha256"]
            ):
...
def run_experiment(shared: Experiment, max_conditions: int | None = None) -> None:
