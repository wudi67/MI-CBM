"""Run the isolated Robot concept-to-label MLP diagnostic on CUDA."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--max-steps", type=int, help="Pause after this many NEW MLP optimizer updates"
    )
...
def main() -> None:
    ...
    if args.plan_only:
        ...
        report(
            f"[cyan]Robot Independent MLP diagnostic[/cyan]\n"
            f"Source: {root}/long_label\n"
            "Frozen concept 300 / quantum label 300 by default.\n"
            f"MLP: 5->16->1 Tanh, 113 parameters; seed={config.seed}; "
            f"epochs={config.epochs}; batch={config.batch_size}; "
            f"lr={config.learning_rate}.\n"
            "Train true concepts only. Evaluate normal / correct foot / correct all.\n"
            "Preserve all 32 Born probabilities; no test read/evaluation.\n"
