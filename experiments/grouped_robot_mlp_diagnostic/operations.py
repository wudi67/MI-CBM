...
def status_table(output: Path, since: float = 0.0) -> tuple[Table, str]:
    table = Table(
        title="Robot concept MLP: frozen VQC, Independent training",
        show_header=False,
    )
    ...
    if "validation" in heartbeat:
        ...
        for name, title in (
            ("measured", "Val normal label accuracy"),
            ("correct_foot_shape", "Val correct foot accuracy"),
            ("correct_all_five", "Val correct all accuracy"),
        ):
            table.add_row(
                title,
                f"{validation[name]['accuracy']:.2%}",
            )
        table.add_row("Val true-concept BCE", f"{validation['direct_true']['bce']:.5f}")
    if not alive and state not in {"complete", "paused", "failed"}:
        table.add_row(
            "Recovery",
            "Worker absent; inspect train.log; "
            "resume restores MLP Adam and sample position",
        )
        state = "absent"
    return table, state
...
def launch(output: Path, session: str, arguments: list[str]) -> None:
    ...
    command = shlex.join(
        [
            sys.executable,
            "-m",
            "experiments.grouped_robot_mlp_diagnostic.operations",
            "worker",
            "--out",
            str(output),
            *arguments,
    ...
    console.print(
        Panel(
            table,
            title="Started Robot Independent concept MLP diagnostic",
            border_style="green",
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "outputs/grouped_robot_mlp_diagnostic/robot_v5_l4_seed0",
    )
    parser.add_argument("--session", default="grouped_robot_mlp_diagnostic")
