...
def expval_density(
    ...
    """Expectation value for density-matrix device (NoiseDevice).

    Mirrors expval: rotate to the observable's diagonal basis, then compute
    the expectation from computational-basis probabilities and eigenvalues.
    Returns a tensor of shape [bsz, len(wires)].
    """
    if not isinstance(qdev, tq.NoiseDevice):
        raise TypeError("expval_density expects a torchquantum.NoiseDevice")
    ...
    # Rotate to diagonalizing basis for each (wire, observable)
    for wire, observable in zip(wires, observables):
        for rotation in observable.diagonalizing_gates():
            rotation(qdev, wires=wire)

    # Probabilities of computational basis after rotation
    probs = qdev.get_probs_1d()  # [bsz, 2^n]
    # Handle complex probabilities by taking real part (for NoiseDevice)
    if probs.is_complex():
        probs = probs.real
    bsz, dim = probs.shape
    device = probs.device
    ...
    expectations = []
    for wire, observable in zip(wires, observables):
        eigvals = observable.eigvals.real.to(device)
        # Map each basis index to the bit at 'wire'
        bit = ((idx >> wire) & 1).to(dtype)
        # eigen per basis: eigvals[0] for bit=0, eigvals[1] for bit=1
        eigen = eigvals[0] * (1 - bit) + eigvals[1] * bit
        res = probs @ eigen  # [bsz]
        expectations.append(res)
